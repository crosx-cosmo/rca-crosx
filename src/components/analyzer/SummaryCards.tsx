  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        {action ? <span className="ml-auto shrink-0 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">{action}</span> : null}
      </div>
      {children}
    </div>
  );
...
export function SummaryCards({ result }: { result: RedirectAnalysis }) {
  ...
  const finalTone: Tone =
    raw === "info" || raw === "neutral" ? "neutral" : raw === "success" ? "success" : raw === "error" ? "error" : "warning";
  ...
  return (
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
      ...
      <Shell icon={Flag} label="Final status" tone={finalTone} index={3}>
        <p className={cn("mt-2 text-2xl font-semibold tabular-nums tracking-tight", toneText[finalTone])}>
          {result.finalStatus ?? "—"}
        </p>
      </Shell>
      <Shell
